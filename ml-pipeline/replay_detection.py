...
_MODEL_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "pretrained_models", "replay_lfcc_gmm.pkl"
)
...
if __name__ == "__main__":
    print(
        "replay_detection.py loaded.\n"
        "To get started:\n"
        "  1. Gather a folder of bonafide (live) clips and a folder of replay "
        "(re-recorded/played-back) clips.\n"
        "  2. Call train_replay_model(bonafide_files=[...], replay_files=[...]).\n"
        "  3. Then detect_replay(waveform) will work, matching the same "
        "calling convention as detect_synthetic_voice()."
    )