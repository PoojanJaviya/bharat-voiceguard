...
if __name__ == "__main__":
    print(
        "scam_intent_detection.py loaded.\n"
        "Quick test example:\n"
        ...
        "\n"
        "First run will download the mDeBERTa-v3-base-mnli-xnli model - "
        "needs internet access once, then it's cached locally.\n"
        "\n"
        "You can also chain it directly after streaming_asr.py:\n"
        "  from streaming_asr import transcribe_file\n"
        "  from scam_intent_detection import detect_scam_intent_from_asr_result\n"
        "  asr_result = transcribe_file('testing_audio_files/raj.wav')\n"
        "  intent_result = detect_scam_intent_from_asr_result(asr_result)\n"
    )